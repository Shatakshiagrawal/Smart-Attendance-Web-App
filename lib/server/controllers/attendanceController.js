...
// FIX: Define a constant for the number of frames in the animation.
const NUM_ANIMATION_CHUNKS = 10;
...
const startAttendance = asyncHandler(async (req, res) => {
    try {
        ...
        // FIX: Use the constant to define the number of chunks.
        const secretChunks = Array.from({ length: NUM_ANIMATION_CHUNKS }, () => generateRandomString(16));
        const expiresAt = new Date(Date.now() + 15 * 60 * 1000); // Set expiry to 15 minutes
        ...
        const attendanceSession = await Attendance.create({
            ...
            qrSession: {
                ...
                numChunks: NUM_ANIMATION_CHUNKS, // FIX: Save the fixed number of chunks to the database.
                expiresAt: expiresAt,
            },
        });
...
const refreshAnimationSequence = asyncHandler(async (req, res) => {
    ...
    // FIX: Generate a new sequence with the SAME number of chunks as the original session.
