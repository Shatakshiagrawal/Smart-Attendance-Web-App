...
const startAttendance = asyncHandler(async (req, res) => {
    try {
        ...
        const expiresAt = new Date(Date.now() + 2 * 60 * 1000); // Increased expiry to 2 mins for stability

        const attendanceSession = await Attendance.create({
            teacher: teacherId, // FIX: Added teacher ID to the session
            semester: semesterId,
            students: semester.students.map(s => s._id), // FIX: Added the list of student IDs
            status: 'live',
            qrSession: {
...
const completeAttendanceSession = asyncHandler(async (req, res) => {
    ...
    // Note: This relies on the Semester model having a teacher ref.
    const session = await Attendance.findById(attendanceId).populate({
        path: 'semester',
        select: 'teacher'
    });
    ...
    if (session.semester.teacher.toString() !== req.user._id.toString()) {
        res.status(403);
